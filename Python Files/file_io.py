...
def dataset_detector(samples,labels):
    images_detector = samples
    labels_detector = (labels!=11).astype(int)
    return images_detector,labels_detector

def dataset_recognizer(samples,labels):
    images_recognizer=samples[(labels!=11).reshape(labels.shape[0])]
    labels_recognizer=labels[labels!=11]
    return images_recognizer,labels_recognizer

def dataset_refactor(samples,labels):
    images_new = []
    labels_new = []
    for i,j in enumerate(samples):
        for m in j:
            images_new.append(m)
        if(type(labels[i][0])==float):
            for k in range(j.shape[0]):
                labels_new.append(11)
        else:
            for z in labels[i][0]:
                labels_new.append(z)
    return np.array(images_new), np.array(labels_new).reshape(-1,1)
...
class FileHDF5(File):
    ...
    def write(self, data, filename, db_name, write_mode="a", dtype="float"):
        ...
        db.create_dataset(db_name, data.shape, dtype=dtype, data=data)
        #dataset[:] = data[:]
        db.close()
