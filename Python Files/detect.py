...
class DigitSpotter:
    ...
    def run(self, image, threshold=0.7, do_nms=True, show_result=True, nms_threshold=0.3):
        ...
        for i,area in enumerate(areas):
            if(area>(0.7*max_area) and (area>500)):
                y_pred_improvised.append(y_pred[i])
                bbs_improvised.append(bbs[i])

        if show_result:
            for i, bb in enumerate(bbs_improvised):
                
                # todo : show module
                y1, y2, x1, x2 = bb
                image = show.draw_box(image, bb, 2)
                msg = "{0}".format(y_pred_improvised[i])
                cv2.putText(image, msg, (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), thickness=2)

            cv2.imshow("MSER + CNN", image)
            cv2.waitKey(0)
        
        return np.array(y_pred_improvised)
